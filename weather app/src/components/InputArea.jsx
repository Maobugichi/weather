...
function InputArea() {
  ...
  async function run(event) {
    ...
    try {
      const result = await axios.get(`http://api.weatherapi.com/v1/forecast.json?key=a951354fb6a44badbb0161031240609&q=${change}&days=${day}`)
 ...
 async function getApi() {
  try {
    updateLoad(true)
    const result = await axios.get("http://api.weatherapi.com/v1/forecast.json?key=a951354fb6a44badbb0161031240609&q=Lagos&days=5")
    ...
    updateForecast((prev) => {
      ...
      const results = await Promise.all(randomCity.map(async (item) => {
        const result = await axios.get(`http://api.weatherapi.com/v1/forecast.json?key=a951354fb6a44badbb0161031240609&q=${item}&days=5`)
        return result.data
      }))
      updateUpper(results)
